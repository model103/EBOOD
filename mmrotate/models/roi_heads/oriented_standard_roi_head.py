...
class OrientedStandardRoIHead(RotatedStandardRoIHead):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            bbox_results = self._bbox_forward_train(x, img, sampling_results,   #改动
                                                    gt_bboxes, gt_labels,
                                                    img_metas)
    ...
    def _bbox_forward_train(self, x, img, sampling_results, gt_bboxes, gt_labels,  #改动
                            img_metas):
        ...
        #                                          gt_labels, self.train_cfg, img)   #改动
        bbox_targets = self.bbox_head.get_targets(sampling_results, gt_bboxes,
                                                  gt_labels, self.train_cfg)   #改动
        loss_bbox = self.bbox_head.loss(bbox_results['cls_score'],
                                        bbox_results['bbox_pred'], rois, img,  #改动
                                        *bbox_targets)
