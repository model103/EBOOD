...
from mmrotate.utils import gradient_extractor, get_nonzero_tensor
...
@weighted_loss
def my_loss(pred, target,**kwargs):  #pre的shape：[num,5], 这里的图片**kwargs的像素值范围不是0-1也不是0-255，而是经过了某种特殊处理的图片
    
    
    assert pred.size() == target.size() and target.numel() > 0
    #loss = torch.abs(pred - target)
    #print('loss形状:',loss.shape)
    gradient, gradient_x, gradient_y = gradient_extractor(kwargs['oringin_img'])
    #A = kwargs['oringin_img'].permute(0,2,3,1).squeeze(0).cpu().numpy() #.permute(0,2,3,1)交换唯独至（H,W,C）
    #cv2.imwrite('img.png', A)
    #print('梯度shape:', gradient.shape)
    #print('预测框：',pred)
    #print('GT框：',target)
    pred_ploy = obb2poly(pred, version='le90')  #[num,8]
    target_ploy = obb2poly(target, version='le90')
    print(kwargs['oringin_img'].shape)
    print(target_ploy)
    split_gradient_x_th, split_gradient_y_th, split_gradient_mask, offset= split_gradient_img(pred_ploy, target_ploy, gradient, gradient_x, gradient_y)  #已经归一化的非零梯度向量，及其位置mask
    #list[tensor[num_g],...], list[tensor[num_g],...], list[tensor[num_g,2],...]
    loss_list = []
    for i in range(len(pred_ploy)):  #计算每个小图的loss
        row_x = split_gradient_x_th[i]
        col_x = torch.unsqueeze(row_x, dim=1)

        row_y = split_gradient_y_th[i]
        col_y = torch.unsqueeze(row_y, dim=1)
        w = col_x * row_x + col_y * row_y
        ...
        #    print('nan小图对应的split_gradient_x_th', split_gradient_x_th[i])
        #    print('nan小图对应的split_gradient_y_th', split_gradient_y_th[i])
            #print('nan小图对应的w, approxi_1, approxi_2', w, approxi_1, approxi_2)
    
    loss = torch.Tensor(loss_list).cuda()
    #print('bbox_loss:', loss)
    #print('bbox_loss的shape:', loss.shape)
...
def split_gradient_img(pred_ploy, target_ploy, gradient, gradient_x, gradient_y):
    ...
    for i in range(len(pred_ploy)):
        ...
        nonzero_split_gradient_x, mask = get_nonzero_tensor(split_gradients, split_gradient_x)  #分割出的梯度小图提取非零部分
        nonzero_split_gradient_y, _ = get_nonzero_tensor(split_gradients, split_gradient_y)
        split_gradient_x_th.append(nonzero_split_gradient_x.squeeze(0))
        split_gradient_y_th.append(nonzero_split_gradient_y.squeeze(0))
        split_gradient_mask.append(mask.squeeze(0))
    ...
    return split_gradient_x_th, split_gradient_y_th, split_gradient_mask, offset
...
def get_approxi(mask, pred,target,offset, k = 15, theta2 = 400): #计算某一个小图的某梯度是否在GT内的近似，以及Predbox相同位置附近的近似
    ...
    for i in range(len(mask)):  #计算每个梯度点i的近似
        ...
        for j in range(len(mask)):
            approxi_ij = math.exp(((guass_x-mask[j][0])**2 + (guass_y-mask[j][1])**2)/(2*theta2))/(2*math.pi*theta2)
...
def get_approxi(mask, pred,target,offset, k = 15, theta2 = 400):
    ...
    d = torch.pow(torch.pow((mask[:,0] - gt_x), 2) + torch.pow((mask[:,1] - gt_y), 2), 0.5)  #d为行向量
    beta_mask__ = mask[:,1].clone()
    beta_mask_ = torch.nn.Threshold(gt_y, 0.1)(mask[:,1])  #比gt_y小的置成0.1
    beta_mask = -10*torch.nn.Threshold(-gt_y, 0.1)(-beta_mask_)  #比gt_y大的置成-0.1, 0.1不可能比gt_y大
    beta = gt_a + beta_mask * torch.acos((gt_x - mask[:, 0])/d)
    d_w = torch.abs(d * torch.cos(beta))
    d_h = torch.abs(d * torch.sin(beta))
    approxi_1 = (1 - torch.sigmoid(-k * (d_w - gt_w) / gt_w)) * (1 - torch.sigmoid(-k * (d_h - gt_h) / gt_h))

    beta_p = torch.atan(torch.tan(beta)*(pred_h*gt_w)/(gt_h*pred_w))
