...
class Grad_FPN(BaseModule):
    ...
    def __init__(self,
        ...
        super(Grad_FPN, self).__init__(init_cfg)
    ...
    def forward(self, inputs, imgs):
        ...
        grad_ls = torch.cat(grad_ls, dim=0)
        ...
        grad_ls_downsapmle = [grad_ls_4, grad_ls_8, grad_ls_16, grad_ls_32, grad_ls_64]
        ...
        outs = [
            self.fpn_convs[i](laterals[i])*grad_ls_downsapmle[i] for i in range(used_backbone_levels) #对融合后的各层分别进行3*3卷积
        ]
        ...
        #方案一：只使用注意力后的FPN
        #方案二：把使用注意力后的FPN和之前的FNP通过卷积进行融合
        #方案三：把使用注意力后的FPN和之前的FNP按元素相乘
